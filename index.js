...
const retryWithBackoff = async (fn, maxRetries = 4, baseDelay = 1000, context = "operation") => {
...
const saveToFirebase = async (bookingData, paymentDetails) => {
  return retryWithBackoff(async () => {
    ...
    const saveData = {
      ...
      slotType: String(bookingData.slotType || 'bookings'),
      status: "booked",
    ...
    console.log("📊 Save data structure:", JSON.stringify(saveData, null, 2));

    // Force use of 'bookings' collection to ensure consistency
    const collectionName = 'bookings';
    console.log(`🔥 Using Firebase collection: ${collectionName}`);
...
// Enhanced payment link creation endpoint
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    if (webhookSignature && webhookSecret) {
      ...
      console.log(`⚠️ [${requestId}] Webhook signature verification skipped (missing signature or secret)`);
    }
...
const handlePaymentLinkPaid = async (paymentLinkEntity, paymentEntity, requestId) => {
  try {
    ...
    console.log(`💾 [${requestId}] Booking data to save:`, JSON.stringify(bookingDataWithPayment, null, 2));
    console.log(`💾 [${requestId}] Payment details to save:`, JSON.stringify(paymentDetails, null, 2));
...
app.get("/health", (req, res) => {
  ...
  res.json({ 
    ...
    version: "4.0 - Enhanced Server-Side Payment System",
...
app.get("/test-firebase", async (req, res) => {
  try {
    ...
    console.log("🧪 Attempting to save test data to Firebase...");
    ...
    res.json({
      success: true,
      message: "Firebase connectivity test successful",
...
app.listen(PORT, () => {
  console.log(`🚀 Enhanced Server-Side Payment System running on port ${PORT}`);
  console.log(`📡 Version: 4.0 - Complete Server-Side Payment Link System`);
  ...
  console.log(`⚡ Enhanced server-side processing with mobile app support`);
});